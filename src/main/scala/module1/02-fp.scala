...
object opt {
  ...
  sealed trait Option[+A] {
     ...
     */
    def orElse[A](anyVal: A): A = this match {
      case Some(v: A) => v
      case None => anyVal
    }
     ...
     */
    def zip[A](optVar: Option[A]): Option[(A, A)] = this match {
      case None => None
      case Some(v: A) =>
        optVar match {
          case Some(optVar: A) => Some(v, optVar)
          case None => None
        }
    }
     ...
     */
    def filter(prediction: A => Boolean): Option[A] = this match {
      case Some(v) if prediction(v) => this
      case None => None
    }
  }
